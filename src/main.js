let localized="en";
const projname=["ポアンカレの円盤","クラインの円盤","上半平面"];
let projid=0;
...
urocheck.src="pic/urocheck.png";
const urogif=[new Image(),new Image(),new Image(),new Image()];
for(let k=0; k<4; ++k){
    urogif[k].src=`pic/Urowalk/uw${k}.png`;
}
...
function render(){
    ...
    ctx.font="12px solid";
    ctx.fillText(`${translate("投影:")}${translate(projname[projid])}`,30,canvas.height-100);
    ctx.fillText(translate("Pで変更"),30,canvas.height-70);
    //description
    ctx.fillText(translate(`右クリックで頂点を選択`),30,canvas.height-230);
    ctx.fillText(translate(`Hで経路探索(最短経路)`),30,canvas.height-200);
    ctx.fillText(translate(`Rで経路リストをすべて削除`),30,canvas.height-170);
                    ...
                    if(s.index[0]==choice || s.index[1]==choice){
                        ...
                        if(choice!=-1){
                            ...
                            for(let k=0; k<pathlist.length; ++k){
                                for(let i=0; i<pathlist[k].length-1; ++i){
                                if((pathlist[k][i]==s.index[0] && pathlist[k][i+1]==s.index[1])){
                                    plind=k;
                                    break;
                                }
                                }
    ...
    for(const v of vertex){
        if(sortedvertex.length==0 || sortedvertex.findIndex(e=>e.index==v.index)!=-1){
...
function geodesic(p,q,limit){
    ...
    ctx.beginPath();
    for(let k=0; k<=d; ++k){
        const c=fix(geo.translate(vectormul(pq,Math.tanh(k/d*Math.atanh(t))/t),q));
        ctx.lineTo(c[0],c[1]);
    }
    ctx.stroke();
    ctx.closePath();
...
function hyperText(text,p){
    ...
    const size=Math.round(base*hyperscale(p.pos,0.3));
        if(size>1){
    const v=fix(p.pos);
...
function projected(p){
    ...
    if(projid==2){
        return projection.upperhalf(p);
    }
}
function translate(raw){
    if(localized=="en"){
        const index=translation.findIndex(e=>e[0]==raw);
        if(index!=-1){
